...
Public/in-repo tests are **evidence, not an oracle**. They often capture intended behavior, but in bug-fix tasks they may encode the legacy bug or be stale relative to the public issue. When a test-derived obligation conflicts with prompt/issue intent, mark it **SUSPECT**, explain the conflict in `SPEC.md` / `FINDINGS.md`, and do not let it veto the public-intent spec by itself.
...
7. If both intent and default convention are unclear, mark the point underspecified and ask an UltimatePowers-style clarification.
