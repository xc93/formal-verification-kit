...
- **Output form / evaluation point**: a value the issue shows **bare** — `f(x) == <closed form>` with no `.expand()` / `.simplify()` / `.doit()` or other opt-in call — is an obligation on the **default / construction path** (an `eval` / `__new__` / constructor classmethod, the operator's own result), not only under an opt-in transform. Decide placement from **what kind of obligation it is**, not from which transform the issue's REPL happened to use: a **value at a specific / concrete argument is an intrinsic constant** and binds on the construction path even when the issue only managed to surface it via `f(x).expand()` — that transform is the user's *workaround* for the missing auto-evaluation, and the bare-unevaluated print beside it is the *symptom*. Only a **symbolic identity in a free variable** (a rewrite that may be undesirable to apply automatically) legitimately binds "only under the transform that shows it." Do not relocate a bare-form obligation onto an opt-in path to keep current behavior, and do not file a specific-point constant next to the symbolic rewrites just because they share a method.
- **Family / table completeness**: when intent describes a *set* of like cases (special values of a function, a rule family, overloads/operators) and shows only one example, the obligation is the **whole known set** — enumerate members from the unit's definition and established domain references, and across any duplicated/generated mirror; a nameable unhandled member (or un-mirrored copy) is a Finding. **Apply** every member you can *derive* from intent or an established domain/math definition (showing the derivation); the absence of an execution layer is not a reason to withhold a soundly-derived value. Reserve an open Finding for members you genuinely cannot derive — never guess a value you cannot derive. Discharge the family **uniformly**: every derived member lands on the **same path the contract requires** (the construction path for specific-point values) and is committed — never apply the familiar/real members while deferring or re-routing the complex ones. A family split across the construction path and an opt-in method, or one where only the easy members were applied, is not discharged.
