...
**When the contract is a *family* of like cases, specify the whole family — not just the member the issue exhibits.** If the intended behavior is a *table* or *set* of homogeneous cases — the special values of a function, a set of implication/inference rules, a group of overloads or operators, a family of formats — and the issue shows only **one** member, the obligation is the **entire known family**, not that single instance. Enumerate the members from the unit's own definition and the established domain/library/math references it implements (e.g. the standard special values of the function being defined), and specify each; a member you can name but did not handle is a Finding, not out of scope. The same applies across **duplicated or generated** copies of the family: if the rule/table is mirrored in a second engine, a generated file (`*_generated.py`, a built cache), or a parallel resolver, the fix must reach every copy — an untouched mirror or un-regenerated cache is a Finding. **Balance — derive, don't guess; a sound derivation is enough to act.** Specify and **apply** every member you can *derive* from intent or an established domain/math definition (show the derivation) — even though the kit cannot execute to confirm it. Constructing such a result *is* the kit's job; **the lack of an execution layer is not a reason to demote a soundly-derived value to a deferred Finding.** Reserve an open Finding for a member you genuinely *cannot* derive (a real gap), and never guess a value you cannot derive. **Discharge the family *uniformly*.** Every member you derive must land on the **same path the contract requires** — the construction / automatic-evaluation path for specific-point values, per the output-form rule below — **and** be committed to code, not parked in a Finding. You may not apply the easy/familiar members while routing the harder/complex ones onto a different path or deferring them: a family split across the construction path and an opt-in method, or one where only the obvious members were handled, is **not discharged** — it is a Finding. A test or contract that exercises the family will probe the member you found least convenient (often the *first* assertion), so partial or heterogeneous placement fails exactly there.
...
The **form** of a desired output is a spec signal too: a value shown bare (no opt-in transform such as `.expand()`) sets a placement obligation on the default/construction path (`eval`/constructor), not an opt-in method. Landing it on an opt-in path while the default path still returns the old value is a Finding, not a confirmation. **Two corollaries decide placement from *semantics*, not from where existing code or the issue's REPL happen to sit:** (a) **a value at a *specific / concrete argument* is an intrinsic constant of the object — it belongs on the automatic-evaluation / construction path and must hold with *no* opt-in call.** Only a *symbolic identity in a free variable* (a rewrite of the function for symbolic input, which may be undesirable to apply automatically) legitimately lives on an opt-in transform. Filing a specific-point value where the symbolic rewrites happen to live (`f(2, 2)` next to `f(1, z) → …`) is a **misfile**: the constant must auto-evaluate even though the symbolic rewrite stays opt-in. (b) **When the issue shows the wanted value *only under* a transform (`f(x).expand() == …`) while bare `f(x)` prints unevaluated, that bare-unevaluated print is the *symptom* and the transform is the user's *workaround* for the missing auto-evaluation — not evidence the value "belongs" under that transform.** Read the workaround as the bug it routes around, and place the value where the bare form requires it (the construction path), not where the issue's REPL happened to surface it.
