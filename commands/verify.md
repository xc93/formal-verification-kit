...
This governs **placement / evaluation-point** choices too. When *where* a value takes effect is under-determined, you may **not** break the tie with a fresh secondary convention (e.g. "this transform is opt-in", "similar values already live in method `M`") to land on the current implementation — an under-determined choice resolved toward V1 is a Finding, not a confirmation. Resolve it from the issue's desired output **form**: a value shown bare (no opt-in call) must hold on the default/construction path. And if you find yourself predicting the chosen placement would fail "if it were tested," that prediction *is* the falsification — switch to the placement that satisfies the bare-form obligation. The placement is **not** under-determined merely because the issue displays the value under a transform: a value at a specific/concrete argument is an intrinsic constant that binds on the construction path, and an issue that shows it via `f(x).expand()` while bare `f(x)` prints unevaluated is exhibiting the *symptom + workaround*, not licensing the opt-in path. "The siblings (`f(1, z) → …`) live in the expansion method" is exactly the forbidden secondary convention — a symbolic rewrite sharing a method does **not** pull a specific-point constant off the construction path.
...
"Constructed, not machine-checked" constrains what you may **claim** (never assert machine-checked confidence) and whether you may **delete tests** (no) — it is **not** license to **withhold a fix you have soundly derived**. If formal or intent reasoning determines the correct value, branch, edit, or member, **apply it to the code** and record the derivation; producing correct results *without running them* is exactly what the kit is for. Demote a derived change to a deferred/open Finding only when the derivation itself is genuinely incomplete or out of reach — **never merely because you cannot execute to double-check it.** **A complex-valued or branch-sensitive result is not automatically "underivable."** When a library documents a branch convention (a principal branch, a canonical cut), the sign/branch of a standard value **follows from that convention** — derive it (cite the convention) and commit it, exactly as you would a real value. "I can't confirm the sign/branch against the numerics (`mpmath`, a reference eval) without executing" is the **execution double-check this rule already says is not grounds to withhold** — do not relabel "unverified against the numerics" as "derivation incomplete" to defer the hard member while committing the easy one. Reserve deferral for when the convention itself is genuinely undocumented or ambiguous in the code/domain, and never guess a sign you cannot derive. (Keep labeling the proof "constructed, not machine-checked," and keep test removal conditioned on the machine check — those honesty rules stand.)
